...
class RuleEngine:
    ...
    def _compile_candidate_bindings(self, rule):
        ...
        for role in required_roles_in_order:
            ...
            role_candidate_pools.append(candidate_pool)
    ...
    def _triggers(self, rule, bindings):
    ...
    def _fire(self, rule, bindings):
        ...
        if self.shuffle_randomly:
            random.shuffle(response_actions)
    ...
    def _spawn_action(self, rule, bindings):
        ...
        action_object = Action(action_name=rule.action_name, action_string=resolved_action_string)
    ...
    def outline(self):
        """Print out all facts in the working memory."""
...
class WorkingMemory:
    ...
    def __init__(self, initial_facts):
        ...
        for initial_fact in initial_facts:
            try:
                self._facts_by_first_character[initial_fact[0]].add(initial_fact)
            except KeyError:
                self._facts_by_first_character[initial_fact[0]] = {initial_fact}
...
class Action:
    ...
    def __init__(self, action_name, action_string):
